...
sage.Svg = class{
	...
	createPath(pathColor,pathWidth,pashDashAry){
		var elm = document.createElementNS(this.ns,"path");
		elm.setAttribute("fill", "none");
		if(pathColor) elm.setAttribute("stroke", pathColor);
		if(pathWidth) elm.setAttribute("stroke-width", pathWidth);
		if(pashDashAry) elm.setAttribute("stroke-dasharray", pashDashAry);
